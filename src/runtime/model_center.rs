...
pub fn semantic_router_topology() -> Result<SemanticRouterTopology, String> {
    if crate::runtime::fixed_models::FIXED {
        return Ok(SemanticRouterTopology::Embedded);
    }
...
pub fn test_model_endpoint(endpoint: &ModelEndpoint) -> ModelEndpointTestResult {
    if crate::runtime::fixed_models::is_fixed_endpoint(endpoint) {
        return test_fixed_model_execution(endpoint);
    }
...
fn test_fixed_model_execution(endpoint: &ModelEndpoint) -> ModelEndpointTestResult {
    use crate::runtime::fixed_models;
    let embedding = endpoint.model_kind == ModelKind::Embedder;
    let (path, body) = if embedding {
        (
            "embeddings",
            json!({"model": fixed_models::EMBEDDING_MODEL, "input": "Health check."}),
        )
    } else {
        (
            "chat/completions",
            json!({"model": fixed_models::CHAT_MODEL,
            "messages": [{"role": "user", "content": "Ready."}], "max_tokens": 1, "temperature": 0}),
        )
    };
    // A retry executes through Beacon so process restart also holds the normal AI lease.
    let result = (|| -> Result<(), String> {
        let token = crate::service_auth::model_api_verifier_token()?;
        let client = Client::builder()
            .timeout(Duration::from_secs(175))
            .build()
            .map_err(|error| error.to_string())?;
        let response = client
            .post(format!("{}/{path}", fixed_models::INFERENCE_BASE))
            .bearer_auth(token.current)
            .json(&body)
            .send()
            .map_err(|error| error.to_string())?;
        ...
        if !status.is_success() {
            return Err(format!(
                "fixed model execution returned HTTP {}",
                status.as_u16()
            ));
        }
        ...
        if !valid {
            return Err("fixed model execution returned an invalid result".into());
        }
        Ok(())
    })();
    ModelEndpointTestResult {
        ok: result.is_ok(),
        status: if result.is_ok() { "active" } else { "degraded" }.into(),
        summary: result
            .err()
            .unwrap_or_else(|| "Fixed local model execution is ready.".into()),
        endpoint: redact_model_endpoint(endpoint),
        details: json!({"local_model_policy": "fixed", "execution_probe": true}),
    }
}
...
fn canonical_embedded_semantic_router_endpoint() -> Result<ModelEndpoint, String> {
    if crate::runtime::fixed_models::FIXED {
        let mut endpoint = AdminModelCenterState::default()
            .endpoints
            .into_iter()
            .find(|endpoint| {
                endpoint.model_endpoint_id == crate::runtime::fixed_models::CHAT_ENDPOINT
            })
            .ok_or("fixed chat endpoint is missing")?;
        set_metadata_bool(
...
mod tests {
    ...
    use super::{
        ...
        openai_compatible_config_from_endpoint, probe_local_runtime_target, redact_model_endpoint,
        resolve_local_runtime_probe_target, run_cat_recording_validation_with_state,
        run_embedding_with_state, run_llm_text_on_endpoint, run_llm_text_with_state,
        run_llm_text_with_state_and_options, run_rerank_with_state, run_vlm_summary_with_state,
        semantic_router_endpoint_for_readiness, semantic_router_local_only_model_state,
        test_model_endpoint, validate_embedded_model_api_base_url, vlm_endpoint_readiness,
        AiWorkload, LlmTextOptions, RERANK_POLICY_ID, SEMANTIC_ROUTER_TOPOLOGY_ENV,
    };
