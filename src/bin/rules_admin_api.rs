...
mod tests {
    use super::*;
    use harborbeacon_local_agent::runtime::admin_console::HomeAssistantConfigUpdate;
    ...
    impl TestApi {
        fn new() -> Self {
            ...
            let api = AdminApi::new(admin, tasks, root.join("webui"), base.clone());
    ...
    fn accepted_ha_action_with_unreadable_response_is_unknown_and_not_retried() {
        ...
        admin
            .save_home_assistant_config(HomeAssistantConfigUpdate {
                ...
                access_token: Some("test-only-ha-token".into()),
                exposed_domains: vec!["light".into()],
            })
            .unwrap();
        let worker = thread::spawn(move || {
            let states = server
                ...
                .unwrap()
                .unwrap();
            assert_eq!(states.url(), "/api/states");
            states
                .respond(Response::from_string(
                    r#"[{"entity_id":"light.test","state":"off","attributes":{}}]"#,
                ))
                .unwrap();
            let action = server
                .recv_timeout(Duration::from_secs(10))
                .unwrap()
                .unwrap();
            assert_eq!(action.url(), "/api/services/light/turn_on");
            action
                .respond(Response::from_string("invalid-json"))
                .unwrap();
            assert!(server
        ...
        assert_eq!(first.actions[0].status, "unknown");
        let replay = rules
            .run(
                &rule.rule_id,
                1,
    ...
    fn state_worker_reprimes_after_disconnect_and_same_second_reenable() {
        ...
        let worker = thread::spawn(move || {
            ...
            .enumerate()
            {
                ...
                match state {
                    Some(state) => {
                        let payload = json!([{"entity_id":"light.test","state":state,"attributes":{},"last_changed":format!("change-{index}")}]);
                        request
